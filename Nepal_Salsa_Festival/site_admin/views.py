...
def dashboard(request):
    ...
    if (is_authenticated):
        ...
        messages.error(request, "You are not authenticated, sign in as a staff")
        return redirect(reverse("signin"))
...
def site_admin_festival_images(request):
    ...
    for festival_image in festival_images:
        festival_images_dict[festival_image.festival.name].append(festival_image)
...
def create_dance_performer(request):
    if request.POST:
        ...
        if dancer_form.is_valid():
            dancer_form.save()
            messages.success(request, "You've Sucessfully Created A Dance Performance")
...
def create_food_vendor(request):
    if request.POST:
        ...
        if food_vendor_form.is_valid():
            ...
            messages.success(request, "You've Sucessfully Created A Food Vendor")
...
def create_festival_image(request):
    if request.POST:
        festival_image_form = AdminFestivalImageForm(request.POST, request.FILES)
        if festival_image_form.is_valid():
            festival_image_form.save()
            messages.success(request, "You've Sucessfully Created A Festival Image")
...
def edit_festival(request, id):
    ...
    if request.POST:
        festival_form = AdminFestivalForm(request.POST, request.FILES, instance=festival)
        if festival_form.is_valid():
            festival_form.save()
            messages.success(request, f"You've Sucessfully Edited {festival.name}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': festival_form.errors}, status=400)
    else:
        festival_json = serializers.serialize("python", [festival], ensure_ascii=False)
...
def edit_schedule(request, id):
    ...
    if request.POST:
        ...
        if schedule_form.is_valid():
            schedule_form.save()
            messages.success(request, f"You've Sucessfully Edited {schedule.act}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': schedule_form.errors}, status=400)
    else:
        schedule_json = serializers.serialize("python", [schedule], ensure_ascii=False)
...
def edit_dance_performer(request, id):
    ...
    if request.POST:
        dancer_form = AdminDancerForm(request.POST, request.FILES, instance=dancer)
        if dancer_form.is_valid():
            dancer_form.save()
            messages.success(request, f"You've Sucessfully Edited {dancer.name}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': dancer_form.errors}, status=400)
    else:
        festival_json = serializers.serialize("python", [dancer], ensure_ascii=False)
        return JsonResponse({"dancer" : festival_json[0]}, status=200)

def edit_musician(request, id):
    musician = Musicians.objects.get(id=id)
    if request.POST:
        musician_form = AdminMusicianForm(request.POST, request.FILES, instance=musician)
        if musician_form.is_valid():
            musician_form.save()
            messages.success(request, f"You've Sucessfully Edited {musician.name}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': musician_form.errors}, status=400)
    else:
        musicians_json = serializers.serialize("python", [musician], ensure_ascii=False)
        return JsonResponse({"musician" : musicians_json[0]}, status=200)

def edit_emcee(request, id):
    emcee = Emcees.objects.get(id=id)
    if request.POST:
        emcee_form = AdminEmceeForm(request.POST, request.FILES, instance=emcee)
        if emcee_form.is_valid():
            emcee_form.save()
            messages.success(request, f"You've Sucessfully Edited {emcee.name}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': emcee_form.errors}, status=400)
    else:
        emcees_json = serializers.serialize("python", [emcee], ensure_ascii=False)
...
def edit_food_vendor(request, id):
    ...
    if request.POST:
        food_vendor_form = AdminEmceeForm(request.POST, request.FILES, instance=food_vendor)
        if food_vendor_form.is_valid():
            food_vendor_form.save()
            messages.success(request, f"You've Sucessfully Edited {food_vendor.name}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': food_vendor_form.errors}, status=400)
    else:
        food_vendors_json = serializers.serialize("python", [food_vendor], ensure_ascii=False)
...
def edit_ticket(request, id):
    ...
    if request.POST:
        ...
        if ticket_form.is_valid():
            ticket_form.save()
            messages.success(request, f"You've Sucessfully Edited {ticket.ticket_type}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': ticket_form.errors}, status=400)
    else:
        tickets_json = serializers.serialize("python", [ticket], ensure_ascii=False)
...
def edit_order(request, id):
    ...
    if request.POST:
        ...
        if order_form.is_valid():
            order_form.save()
            messages.success(request, f"You've Sucessfully Edited {order.order_name}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': order_form.errors}, status=400)
    else:
        orders_json = serializers.serialize("python", [order], ensure_ascii=False)
        return JsonResponse({"order" : orders_json[0]}, status=200)

def edit_sponsor(request, id):
    sponsor = Sponsor.objects.get(id=id)
    if request.POST:
        sponsor_form = AdminSponsorForm(request.POST, request.FILES, instance=sponsor)
        if sponsor_form.is_valid():
            sponsor_form.save()
            messages.success(request, f"You've Sucessfully Edited {sponsor.email}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': sponsor_form.errors}, status=400)
    else:
        sponsors_json = serializers.serialize("python", [sponsor], ensure_ascii=False)
        return JsonResponse({"sponsor" : sponsors_json[0]}, status=200)

def edit_volunteer(request, id):
    volunteer = Volunteer.objects.get(id=id)
    if request.POST:
        volunteer_form = AdminVolunteerForm(request.POST, request.FILES, instance=volunteer)
        if volunteer_form.is_valid():
            volunteer_form.save()
            messages.success(request, f"You've Sucessfully Edited {volunteer.email}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': volunteer_form.errors}, status=400)
    else:
        volunteers_json = serializers.serialize("python", [volunteer], ensure_ascii=False)
...
def edit_feedback(request, id):
    ...
    if request.POST:
        ...
        if feedback_form.is_valid():
            feedback_form.save()
            messages.success(request, f"You've Sucessfully Edited {feedback.first_name} {feedback.last_name}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': feedback_form.errors}, status=400)
    else:
        feedbacks_json = serializers.serialize("python", [feedback], ensure_ascii=False)
...
def edit_festival_image(request, id):
    ...
    if request.POST:
        festival_image_form = AdminFestivalImageForm(request.POST, request.FILES, instance=festival_image)
        if festival_image_form.is_valid():
            festival_image_form.save()
            messages.success(request, f"You've Sucessfully Edited {festival_image.image_title}!")
            return JsonResponse({"success" : True})
        else:
            return JsonResponse({'errors': festival_image_form.errors}, status=400)
    else:
        festival_image_json = serializers.serialize("python", [festival_image], ensure_ascii=False)
...
def delete_ticket(request, id):
    try:
        ticket = Ticket.objects.get(id=id)
        ticket.delete();
        messages.success(request, f"{ticket.ticket_type}, sucessfully deleted!")
        return JsonResponse({"success" : True})
    except:
        return JsonResponse({"error" : "Not Found"}, status=404)
...
def delete_feedback(request, id):
    try:
        feedback = Feedback.objects.get(id=id)
        feedback.delete();
        messages.success(request, f"{feedback.first_name} {feedback.last_name}, sucessfully deleted!")
        return JsonResponse({"success" : True})
    except:
        return JsonResponse({"error" : "Not Found"}, status=404)
...
def delete_festival_image(request, id):
    try:
        ...
        messages.success(request, f"{festival_image.image_title}, sucessfully deleted!")
        return JsonResponse({"success" : True})
    except:
        return JsonResponse({"error" : "Not Found"}, status=404)
